...
caption = "Bubble plot of length compositions by year and fleet. Size of the bubble indicates higher proportion. All proportions within year sum to one",
label = 'comm-rec-lts_bubbs')
```
...
```{r, results = 'asis'}
add_figure(
filein = file.path(index_fig_loc, "OR_ORBS_Index_S-M_coef.png"), 
caption = "Species coefficients (blue bars) for presence/absence of vermilion rockfish in the ORBS dockside index. Horizontal black bars represent the 95 percent confidence intervals",
label = 's-m-coef')
```	

```{r, results = 'asis'}
add_figure(
filein = file.path(index_fig_loc, "OR_ORBS_Index_S-M_ROC.png"), 
...
label = 'OR-ORBS-Index-S-M-ROC')
```	

```{r, results = 'asis'}
add_figure(
filein = file.path(index_fig_loc, "ORBS_DataSummary.png"), 
caption = "Characterization of the final subset of ORBS dockside data used in the delta-GLM analyses to develop an index for vermilion rockfish",
label = 'ORBS-DataSummary')
```	

```{r, results = 'asis'}
add_figure(
filein = file.path(index_fig_loc, "ORBS_CPUESummary.png"), 
caption = "The distribution of trip-level raw positive catch CPUE for the ORBS dockside data relative to potential covariates evaluated in the vermilion rockfish delta-GLM analysis",
label = 'ORBS-CPUESummary')
```	

```{r, results = 'asis'}
add_figure(
filein = file.path(index_fig_loc, "ScaledQQplot_bin.png"), 
caption = "Scaled quantile-quantile plot (left) and rank-transformed versus standardized
residuals (right) for the binomial model of the ORBS dockside index",
label = 'ScaledQQplot-bin')
```	

```{r, results = 'asis'}
add_figure(
filein = file.path(index_fig_loc, "ScaledQQplot_pos.png"), 
caption = "Scaled quantile-quantile plot (left) and rank-transformed versus standardized
residuals (right) for the positive model of the ORBS dockside index",
label = 'ScaledQQplot-pos')
```	

```{r, results = 'asis'}
add_figure(
filein = file.path(index_fig_loc, "ORBS_OR_Dockside_Index.png"), 
caption = "Final ORBS dockside index for vermilion rockfish",
label = 'ORBS-OR-Dockside-Index')
```	

```{r, results = 'asis'}
add_figure(
filein = file.path(index_fig_loc, "prop_zeros_bin_interaction.png"), 
caption = "Comparison of data distribution for vermilion rockfish CPUE to model-generated replicate data sets used to evaluate uncertainty for the binomial model component",
label = 'prop-zeros-bin-interaction')
```	

```{r, results = 'asis'}
add_figure(
filein = file.path(index_fig_loc, "prop_zeros_pos.png"), 
caption = "Comparison of data distribution for vermilion rockfish CPUE to model-generated replicate data sets used to evaluate uncertainty for the positive model component",
label = 'prop-zeros-pos')
```	
...
caption = "Ageing error matrix (age by standard deviation (SD)) values by source. The commercial and recreational matrices are based on intra-reader comparisons",
label = 'age-error')
```
...
caption = "Composite natural mortality distribution for vermilion rockfish using four longevity estimators each with a SD = 0.438 presuming a lognomral error distibution",
...
caption = "Sex-specific length (cm)-weight (kg) data and fits to commercially-derived vermilion rockfish samples",
...
caption = "Mean length (cm) index from the commercial fishery with 95 percent confidence intervals based on sample sizes and data weighting",
...
caption = "Mean age from conditional age-at-length data for the commercial fishery",
label = 'com-mean-caal')
```
